                  ...
                  <table class="table">
                     ...
                     <?php foreach($students as $i):?>
                        <tr>
                           ...
                           <td><?= $i->mname?></td>
                           <td><?= $i->lname?></td>
                           <td class="<?= ($i->status == 0) ? 'text-danger' : 'text-success'?>">
                              <?= ($i->status == 0) ? 'Inactive' : 'Active'?>
                           </td>
                           <td><?= $i->grade_title?></td>
                           <td>
                              <a href="<?=base_url('View_student/'.$i->userID)?>" class="btn"><i class="material-icons">search</i></a>
                              <a href="<?=base_url('Edit_student/'.$i->userID)?>" class="btn"><i class="material-icons">edit</i></a> 
...
<div class="modal fade" id="exampleModal<?=$i->userID?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <?=form_open("Admin/delete_student/$i->userID")?>
